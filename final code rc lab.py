...
#    set3C = ["Set 3C Vin.txt", "Set 3C Vout.txt"]
    ...
    # creating x-axis with a delta T of 0.4 microseconds
    ...
    for VinValue in VinValues:
        ...
        VoutModel.append(nextVout-6.5)
        initialVout = nextVout
        
    #deleting last element to fix index problem
    del VoutModel[-1]
    ...
    for i in range(3199):
        chiSquare = (VoutValues[i]-VoutModel[i]/0.1)**2
        chisq = chisq + chiSquare
    
    dof = 3200-5     #3200 data values minus two constraints (resistance and capacitance)
